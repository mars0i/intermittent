...
(defn make-mst
  ...
  ([] (make-mst (make-long-seed)))
  ([long-seed] 
   (let [rng (MersenneTwisterFast. long-seed)] ; note use of MersenneTwister vs MersenneTwisterFast
     (flush-mst rng)
     rng))) 
...
(defn pink-nums
  ...
  [pink] (repeatedly (partial next-pink pink)))
...
(defn logistic-pink-nums
  "Return a lazy sequence of numbers generated by PinkNoise object pink and then
  restricted to [0,1] by passing them through normal-logistic with the specified
  growth-rate."
  [pink growth-rate] (repeatedly (comp (partial normal-logistic growth-rate)
                                       (partial next-pink pink))))
